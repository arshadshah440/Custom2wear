jQuery(document).ready(function ($) {
  ...
  jQuery("#show_mi").on("change", function (e) {
    ...
    if (selectedValue === "all") {
        ...
        jQuery("#showing_values_mi").find("p").html(`Showing all <span>${selectedValue}</span> results`);
    }
});
...
if (window.location.href.indexOf("shop") > -1) {
    ...
    if (numofpost) {
        ...
        if (numofpost === "all") {
            ...
            jQuery("#showing_values_mi").find("p").html(`Showing all <span>${numofpost}</span> results`);
        }
    }
}
